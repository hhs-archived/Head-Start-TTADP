...
const processRecords = async (
  ...
  try {
    ...
    auditLogger.info('error', ` processRecords getNextObject ${err.message}`);
  }
  ...
  try {
    ...
    recordActions.errors.push(err.message);
    auditLogger.info('error', ` processRecords modelForTable ${err.message}`);
  ...
  if (record) {
    try {
      ...
      // other entries may be process successfully
      recordActions.errors.push(err.message);
      auditLogger.info('error', ` processRecords create/update ${err.message}`);
    }
  } else {
    try {
      ...
      auditLogger.info('error', ` processRecords destroy ${err.message}, ${JSON.stringify(err)}`);
    }
...
const processFile = async (
  ...
  try {
    ...
    auditLogger.info('error', ` processFile ${err.message}`);
  }
...
const processFilesFromZip = async (
  ...
  try {
    ...
    auditLogger.info('error', ` processFilesFromZip ${err.message}`);
  }
...
const processZipFileFromS3 = async (
  ...
  try {
    ...
    auditLogger.info('error', ` processZipFileFromS3 downloadFileAsStream ${err.message}`);
    return {
      error: err.message,
  ...
  try {
    ...
    // If an error occurs, set the import file status to PROCESSING_FAILED
    await setImportFileStatus(importFileId, IMPORT_STATUSES.PROCESSING_FAILED);
    auditLogger.info('error', ` processZipFileFromS3 getAllFileDetails ${err.message}`);
    return {
      error: err.message,
      duration: new Date().getTime() - startTime.getTime(),
    };
  ...
  try {
    ...
    // If an error occurs, set the import file status to PROCESSING_FAILED
    await setImportFileStatus(importFileId, IMPORT_STATUSES.PROCESSING_FAILED);
    auditLogger.info('error', `processZipFileFromS3 processFilesFromZip ${err.message}`);
    return {
      error: err.message,
      file: {
