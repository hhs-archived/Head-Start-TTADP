 ...
 *    AWS access key ID to use when signing requests (required for AWS Elasticsearch).
 * @param {string|undefined} secretKey
 *    AWS secret key to use when signing requests (required for AWS Elasticsearch).
 */

/**
 * @param {NodeJS.ProcessEnv|Dict<string>|undefined} env
 *    Environment variables to consider (defaults to `process.env`).
 * @returns {ElasticsearchConfiguration}
 */
export default function getClientConfiguration(env = process.env) {
  ...
  const services = typeof env.VCAP_SERVICES === 'string'
